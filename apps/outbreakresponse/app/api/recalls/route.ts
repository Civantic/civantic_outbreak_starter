...
export async function GET(req: Request){
  ...
  try{
    ...
    const items:Array<any>=Array.isArray(j.results)?j.results:[]
    ...
    const demo=(await import("../../data/demo")).recalls
    let rows:Row[]=demo.map(x=>({ id:x.id,date:x.date,stateScope:x.stateScope||[],product:x.product||"",reason:x.reason||"",classification:undefined,source:"FDA openFDA" }))
    if(scope==="NM") rows=rows.filter(r=>r.stateScope.includes("NM")||r.stateScope.includes("National"))
    if(stateParam) rows=rows.filter(r=>r.stateScope.includes(stateParam)||r.stateScope.includes("National"))
    if(cls) rows=rows.filter(r=>String(r.classification||"").toLowerCase().includes(cls))
    if(pq) rows=rows.filter(r=>String(r.product||"").toLowerCase().includes(pq))
    return NextResponse.json({ data: rows, fallback:true, fetchedAt:new Date().toISOString() })
  }
}
