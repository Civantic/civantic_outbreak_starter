...
export async function GET(req:Request){
  ...
  try{
    ...
    if(!r.ok){
      const res = NextResponse.json({ series:[], error:true, errorDetail:`NWSS ${r.status}: ${text.slice(0,300)}`, fetchedAt:new Date().toISOString() })
    ...
    const res = NextResponse.json({ series:[], error:true, errorDetail:String(e?.message||e), fetchedAt:new Date().toISOString() })
    res.headers.set("Cache-Control","s-maxage=60, stale-while-revalidate=120")
    return res
  }
}
