...
export default function StatePage({ params }: { params: { abbr: string } }) {
  ...
  const qDomain = (q.get("domain") === "foodSafety" ? "foodSafety" : "publicHealth") as "publicHealth" | "foodSafety"
        ...
        if (!cancel) {
          const obRows: Outbreak[] = (ob.data || [])
            .map((x: any) => ({
              ...
              etiology: x.etiology || "Unknown",
              illnesses: Number(x.illnesses || 0),
              hospitalizations: Number(x.hospitalizations || 0),
          ...
          const fdaRows: Recall[] = (fda.data || [])
            .map((x: any) => ({
              id: x.id,
              date: x.date,
              stateScope: (x.stateScope || []).map((s: string) => String(s).toUpperCase()),
              product: x.product || "",
              reason: x.reason || "",
