...
export default function WastewaterPage() {
    ...
    async function load() {
      ...
      try {
        const [rUS, rNM] = await Promise.all([
          fetch(`/api/wastewater?scope=US&months=${monthsBack}`).then(r=>r.json()).catch(()=>({series:[]})),
          fetch(`/api/wastewater?scope=NM&months=${monthsBack}`).then(r=>r.json()).catch(()=>({series:[]}))
        ])
        if (!cancel) { setUS(rUS); setNM(rNM); setLoading(false) }
  ...
  function onMove(e: React.MouseEvent) {
    ...
    for (const set of sets) for (const p of set.pts) {
      const d = Math.abs(px - p.x)
      if (d < min) { min = d; best = { series:set.series, x:p.x, y:p.y, d:p.d } }
    }
    setHover(best)
  }
    ...
    <section className="container py-16 md:py-20">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-2 text-gray-700">
          This shows the <strong>wastewater percentile (0–100)</strong> vs each site’s own history (not case counts).
          Higher = more signal relative to baseline.
        </p>
